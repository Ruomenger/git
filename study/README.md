...
- [学习计划](learning-plan.md)：结合个人背景制定的四阶段路线、每次学习的验证循环、练手仓库与 neovim/clangd/LLDB 环境准备，以及进度记录表。先读这篇确定路线。
- [架构与核心概念导览](architecture-overview.md)：对象库、引用、索引三大抽象，源码分层地图，以及 `git commit` 的完整调用链路。建议作为第一篇正文。
